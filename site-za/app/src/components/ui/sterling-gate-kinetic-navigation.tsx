/* Sterling Gate kinetic navigation (21st.dev), copied as supplied by the client on 25 September
 * 2026 for reference. It is not rendered on the site: the site's adaptation is
 * src/components/site/DoorBoard.tsx (the door list) plus the layered mobile menu in
 * src/site/wayfinding.css. Only type-level edits were made so the file passes the build's strict
 * type check and lint: the button text and icon lookups fall back to an empty list or null instead
 * of undefined, the cleanup handle is typed instead of `any`, and the unused default React import
 * is dropped (the project uses the automatic JSX runtime). Its companion stylesheet was not
 * part of the snippet; the CSS variables it expects are mapped to the brand on .c-board in
 * wayfinding.css. */
...
type WithCleanup = Element & { _cleanup?: () => void };
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (containerRef.current) {
        ...
        items.forEach((item) => (item as WithCleanup)._cleanup?.());
      }
    };
  }, []);
