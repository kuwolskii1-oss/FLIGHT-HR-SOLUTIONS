...
# are left alone. Arguments: SRC_ZIP_URL CHECKOUT_PATH [COMMIT_MESSAGE]
set -euo pipefail
SRC_ZIP_URL=$1; R=$2; MSG=${3:-"Mirror site-za/app from the FHS-2 branch"}
cd "$R"
...
git -c user.name="Flight Hour Solution build" -c user.email="build@flighthoursolution.com" commit -q -m "$MSG" || echo "nothing to commit"
# -3 rather than a pipe into head: under pipefail, head closing the pipe fails the script.
git log --oneline -3
