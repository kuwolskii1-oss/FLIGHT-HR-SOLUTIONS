...
echo "== 3 raw hexes outside tokens.css"; grep -rnoiE "#[0-9a-f]{6}\b" $A/src/site/site.css $A/src/site/za.css $A/src/site/transitions.css | head
...
echo "== 6 reduced motion coverage"; grep -c "prefers-reduced-motion" $A/src/site/site.css $A/src/site/za.css $A/src/site/transitions.css $A/src/components/site/Ident.tsx
...
echo "== 13 animations longer than 600 ms in site css"; grep -noE "[0-9]{3,4}ms" $A/src/site/site.css $A/src/site/za.css $A/src/site/transitions.css $A/src/site/tokens.css | awk -F: '{ if ($3+0 > 600) print }' | head
