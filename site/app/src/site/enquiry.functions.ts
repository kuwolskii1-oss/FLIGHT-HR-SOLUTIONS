...
const enquirySchema = z.object({
  ...
  assetType: z.string().trim().max(80).optional().default(""),
  assetModel: z.string().trim().max(160).optional().default(""),
  engineFamily: z.string().trim().max(80).optional().default(""),
  need: z.string().trim().max(120).optional().default(""),
  message: z.string().trim().max(5000).optional().default(""),
  subject: z.enum(["Website enquiry", "Asset request"]).optional().default("Website enquiry"),
...
export const submitEnquiry = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const lines = [
      ...
      data.assetType ? `Asset type: ${data.assetType}` : "",
      data.assetModel ? `Engine or aircraft type: ${data.assetModel}` : "",
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [to],
        reply_to: data.email,
        subject: `${data.subject} from ${data.name} (${data.company})`,
        text: lines.join("\n"),
